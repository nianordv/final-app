...
def render():
    ...
    with col_trials:
        n_trials = st.slider(
            ...
            value=20,
            step=5,
            help="How many different settings combinations Optuna will try.",
        )
    ...
    with col_folds:
        ...
        st.caption(f"Each trial is evaluated across **{cv_folds}** data splits.")
    ...
    st.markdown("---")
    st.markdown(f"**Ready to run:** {model_name} · {n_trials} trials · {cv_folds}-fold CV")
    ...
    if run_clicked:
        ...
        st.session_state.update({
            ...
            "tune_best_model": best_model,
            "tune_scaler": scaler,
            "tune_encoded_feature_names": encoded_feature_names,
            "tune_original_features": features,
            "tune_df_clean": df_clean,
            "tune_model_uses_scaler": model_uses_scaler,
        })
    ...
    if st.session_state.get("tune_ready"):
        ...
        with col_mae:
            mae_display = (
                f"${mae_val:,.0f}"
                if any(k in _target.lower() for k in cost_kw)
                else f"{mae_val:,.2f}"
            )
            st.metric("Test MAE", mae_display)
        ...
        with col_rmse:
            ...
            st.metric("Test RMSE", rmse_display)
            st.caption(_rmse_caption(rmse_val, _target, _y_full))

        st.markdown("---")
        st.markdown("## Predict Cost")
        st.caption("Select feature values below and the trained model will predict the cost.")

        trained_model = st.session_state["tune_best_model"]
        trained_scaler = st.session_state["tune_scaler"]
        encoded_cols = st.session_state["tune_encoded_feature_names"]
        original_features = st.session_state["tune_original_features"]
        df_for_inputs = st.session_state["tune_df_clean"]
        uses_scaler = st.session_state["tune_model_uses_scaler"]
        ...
        for i, feature in enumerate(original_features):
            with input_cols[i % 2]:
                if pd.api.types.is_numeric_dtype(df_for_inputs[feature]):
                    min_val = float(df_for_inputs[feature].min())
                    max_val = float(df_for_inputs[feature].max())
                    mean_val = float(df_for_inputs[feature].mean())
                    ...
                    options = sorted(df_for_inputs[feature].dropna().unique().tolist())
        ...
        if uses_scaler:
            input_ready = trained_scaler.transform(input_encoded.values)
        else:
            input_ready = input_encoded.values

        if st.button("Predict Cost", type="primary", use_container_width=True):
            prediction = trained_model.predict(input_ready)[0]

            if any(k in _target.lower() for k in cost_kw):
                st.success(f"Predicted {_target}: ${prediction:,.2f}")
            else:
                st.success(f"Predicted {_target}: {prediction:,.2f}")

            with st.expander("View selected input"):
        ...
        with col_reset:
            if st.button("🗑️ Reset all results"):
                for key in [
                    ...
                    "tune_best_model",
                    "tune_scaler",
                    "tune_encoded_feature_names",
                    "tune_original_features",
                    "tune_df_clean",
                    "tune_model_uses_scaler",
                ]:
